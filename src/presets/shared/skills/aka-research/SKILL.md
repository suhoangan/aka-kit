...
1. **Search Strategy**:
   - **Gemini Toggle**: Check `~/.cursor/.ck.json` or `~/.claude/.ck.json` for `skills.research.useGemini` (default: `true`). If `false`, skip Gemini and use WebSearch.
   - **Gemini Model**: Read from the same `.ck.json`: `gemini.model` (default: `gemini-3-flash-preview`)
