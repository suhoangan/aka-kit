...
const BINARIES = [
	{ name: 'node', level: 'required', detail: 'CLI runtime' },
	{
		name: 'npx',
		level: 'required',
		detail: 'used by npm-based MCP servers (playwright, figma, agent-browser)',
	},
	{
		name: 'git',
		level: 'required',
		detail: 'graph-init, husky hooks, repo detection',
	},
	{
		name: 'rtk',
		level: 'recommended',
	...
	},
	{
		name: 'pipx',
		level: 'recommended',
		detail: 'installs code-review-graph + graphify',
		fix: 'See https://pipx.pypa.io/stable/installation/',
	},
	{
		name: 'uv',
		level: 'recommended',
		detail: 'installs serena MCP + spec-kit',
		fix: 'curl -LsSf https://astral.sh/uv/install.sh | sh',
	},
	{
		name: 'uvx',
		level: 'recommended',
		detail: 'used to launch serena MCP',
		fix: 'Comes with uv',
	},
	{
		name: 'specify',
		level: 'optional',
		detail: 'GitHub spec-kit CLI',
		fix: 'uv tool install specify-cli',
	},
	{
		name: 'agent-browser',
		level: 'optional',
	...
	},
	{
		name: 'gh',
		level: 'optional',
		detail: 'GitHub CLI for PR / release flows',
		fix: 'https://cli.github.com/',
	},
	{
		name: 'cargo',
		level: 'optional',
		detail: 'Rust toolchain — fallback installer for rtk / agent-browser',
	},
];
			...
			? [
					{
						name: 'brew',
						level: 'optional',
						detail: 'macOS package manager — fastest install path',
					},
				]
			: []),
		...(!isWin
			? [
					{
						name: 'bash',
						level: 'recommended',
						detail: 'required for dependency install scripts',
					},
				]
			: []),
	];

	// Windows: dependency scripts run via Node; bash optional
	if (isWin) {
		results.push(
			result(
				'Binaries',
				'bash',
				'skip',
				'Optional on Windows — aka-kit uses Node dependency installers',
				'Install Git Bash only for manual .sh scripts',
			),
		);
	}
